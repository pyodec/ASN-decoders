...
class RaobDecoder(FileDecoder):
    ...
    def decode_proc(self, fpath, length, **kwargs):
        ...
        with self.open_ascii(fpath) as g:
            ...
            # THERE ARE NO FIXED VARIABLES, variable dimensions are variables themselves
            i=0
            # loop through the stations we found
            for wmoid in src.variables['wmoStaNum']:#map(''.join,src.variables[src.idVariables]):
                data = {}
                self.state['identifier'] = wmoid
                ...
                row = [src.variables[src.timeVariables][i]] # THIS IS SYNOPTIC TIME!!! (release time is not so reliable)
