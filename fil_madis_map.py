        ...
        with self.open_ascii(fpath) as g:
            ...
            for stnid in map(''.join,src.variables[src.idVariables]):
                ...
                if len(lvls[:]) == 0 or beams == 0:
                    ...
                    # skip this station
                    continue
                # store height as a VARIABLE, because it is, sadly. This is a duplicate of levels, just for naming convention
                self.vars.addvar('HEIGHT','Observation Altitude','float32',lvls[:].shape,"m")
                row.append(lvls[:])
...
if __name__ == "__main__":
    import sys
    fil = sys.argv[1]
    for o in decoder.decode(fil,generator=True):
        if decoder.state['identifier'] != "587JP": continue
        print decoder.state['identifier']
        for v in decoder.get_fixed_vars():
            print v['data'].shape, v['name'], v['data']